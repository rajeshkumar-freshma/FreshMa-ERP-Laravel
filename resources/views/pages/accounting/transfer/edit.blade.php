<x-default-layout>
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Transfer'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="transfer" class="collapse show">
            <!--begin::Form-->
            <form id="transfer_form" class="form" method="POST"
                action="{{ route('admin.transfer.update', $transfer->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                    ...
                    <div class="row mb-3">
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="from_account_id" id="from_account_id"
                                            ...
                                            data-placeholder="{{ __('Select Account Type..') }}"
                                            ...
                                            @foreach ($accounts as $key => $value)
                                                ...
                                                <option value="{{ $value->id }}"
                                                    {{ $value->id == $transfer->from_account_id ? 'selected' : '' }}>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="to_account_id" id="to_account_id"
                                            ...
                                            @foreach ($accounts as $key => $value)
                                                <option value=""></option>
                                                <option value="{{ $value->id }}"
                                                    {{ $value->id == $transfer->to_account_id ? 'selected' : '' }}>
                                                    {{ $value->account_holder_name }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="decimal" name="available_balance"
                                    value="{{ old('available_balance', $transfer->available_balance) }}"
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="decimal" name="transfer_amount"
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="transaction_date" type="text"
                                        ...
                                        value="{{ old('transaction_date',$transfer->transaction_date) }}" />
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                <label for="transfer_reason"
                                    class="required form-label">{{ __('Transfer Reasons') }}</label>
                                <input type="text" name="transfer_reason"
                                    value="{{ old('transfer_reason', $transfer->transfer_reason) }}"
                                    class="form-control form-control-sm form-control-solid" id="transfer_reason"
                                    placeholder="Enter Transfer Reason" required />
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <textarea name="notes" class="form-control form-control-sm form-control-solid" id="notes"
                                    placeholder="Enter Notes">{{ old('notes', $transfer->notes) }}</textarea>
                                @if ($errors->has('notes'))
                                    <span class="fv-plugins-message-container invalid-feedback" role="alert">
                                        ...
                                        <select name="status" id="status" aria-label="{{ __('Select Status') }}"
                                            ...
                                            @foreach (config('app.statusinactive') as $key => $value)
                                                <option value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('status', $transfer->status) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}
                                                </option>
                                            @endforeach
                                        </select>
