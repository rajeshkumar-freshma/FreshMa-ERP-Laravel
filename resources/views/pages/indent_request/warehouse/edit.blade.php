<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            'title' => 'Edit Warehouse Indent Request',
            'menu_1_link' => route('admin.warehouse-indent-request.index'),
            'menu_2' => 'Edit Warehouse Indent Request',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Warehouse Indent Request'])
        <!--begin::Card header-->
    </div>
    <!--begin::Content-->
    <!--begin::Form-->
    <form id="warehouse_indent_request_form" class="form" method="POST"
        action="{{ route('admin.warehouse-indent-request.update', $warehouse_indent_request->id) }}"
        ...
        <div id="warehouse_indent_request">
            <div class="card">
                ...
                <div class="card-body border-top px-9 py-4">
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <input type="text" class="form-control form-control-sm form-control-solid"
                                    id="wir_code" name="wir_code"
                                    value="{{ old('wir_code', $warehouse_indent_request->request_code) }}"
                                    placeholder="WIR-" required />
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="ir_request_date" type="text"
                                        ...
                                        name="request_date"
                                        value="{{ old('request_date', $warehouse_indent_request->request_date) }}"
                                        required />
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="ir_expected_date" type="text"
                                        class="form-control form-control-sm form-control-solid fsh_flat_datepicker"
                                        name="expected_date"
                                        value="{{ old('expected_date', $warehouse_indent_request->expected_date) }}"
                                        required />
                                    <span class="input-group-text border-0">
                                        <i class="fas fa-calendar"></i>
                                    </span>
                                        ...
                                        <select name="warehouse_id" id="ir_warehouse"
                                            ...
                                            @foreach ($warehouses as $key => $warehouse)
                                                <option data-kt-flag="{{ $warehouse->id }}"
                                                    value="{{ $warehouse->id }}"
                                                    {{ $warehouse->id == old('warehouse_id', $warehouse_indent_request->warehouse_id) ? 'selected' : '' }}>
                                        ...
                                        <select name="status" id="po_status" aria-label="{{ __('Select Status') }}"
                                            ...
                                            @foreach (config('app.purchase_status') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('status', $warehouse_indent_request->status) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="ir_file" class="form-label">{{ __('Attachments') }}
                                    {!! commoncomponent()->attachment_view($warehouse_indent_request->image_full_url) !!}</label>
                    ...
                    @include('pages.partials.product_search.index', [
                        'loop_data' => old(
                            'products',
                            $warehouse_indent_request->warehouse_indent_product_details),
                        'main_data' => $warehouse_indent_request,
                        'supplier' => $warehouse_indent_request->supplier_id,
                    ])
                    <!--End::Product Search-->
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-md-12">
                            <div class="mb-5">
                                <label for="remarks" class="required form-label">{{ __('Remarks') }}</label>

                                <textarea class="form-control form-control-sm form-control-solid" name="remarks" id="remarks" cols="30"
                                    rows="3">{{ old('remarks', $warehouse_indent_request->remarks) }}</textarea>
                            </div>
            ...
            {{-- @include('pages.partials.transport_detail', ['transport_detail' => $warehouse_indent_request->transport_details]) --}}
            ...
            @include('pages.partials.form_footer', [
                'is_save' => false,
                'back_url' => route('admin.warehouse-indent-request.index'),
            ])
            <!--end::Actions-->
        </div>
        <!--end::Content-->
