<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_2' => 'Edit Cash Register',
        ])
    @endsection
    <!--begin::Basic info-->
    <form id="purchase_order_return_form" class="form" method="POST"
        action="{{ route('admin.cash-register.update', $cash_register->id ?? '') }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card mt-2">
            <!--begin::Card header-->
            ...
            @foreach (@$store as $key => $value)
                @if (@$cash_register->store_id == $value->id)
                    <b>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="card mb-3 bg-secondary text-white">
                                <div class="card-body">
                                    <p class="mb-0">Opening Cash: {{ @$cash_register->total_amount ?? 0 }}</p>
                                </div>
                            </div>
                        </div>
            ...
            <div id="purchase_order_return">
                ...
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="is_opened" id="is_opened" aria-label="{{ __('Select Store') }}"
                                            ...
                                            <option value=1
                                                {{ old('is_opened', @$cash_register->is_opened) == 1 ? 'selected' : '' }}>
                                                {{ __('Opened') }}
                                            </option>
                                            <option value=2
                                                {{ old('is_opened', @$cash_register->is_opened) == 2 ? 'selected' : '' }}>
                                                {{ __('Closed') }}
                                            </option>
                                        </select>
                                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <input id="open_close_time" type="text"
                                            value="{{ old('open_close_time', @$cash_register->open_close_time) }}"
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <input id="amount" type="text" class="form-control form-control-solid p-2"
                                            value="{{ old('amount', @$cash_register->amount) }}" name="amount"
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="add_dedect_amount" class="form-label">{{ __('Add Dedect Amount') }}</label>
                                <div class="input-group input-group-sm flex-nowrap">
                                    <span class="input-group-text border-0"><i class="fas fa-money"></i></span>
                                    <div class="overflow-hidden flex-grow-1">
                                        <input id="add_dedect_amount" type="text"
                                            class="form-control form-control-solid p-2" name="add_dedect_amount"
                                            value="{{ old('add_dedect_amount', @$cash_register->add_dedect_amount) }}"
                                            placeholder="Open Close Time" />
                                    </div>
                                </div>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <input id="total_amount" type="text"
                                            ...
                                            value="{{ old('total_amount', @$cash_register->total_amount) }}"
                                            placeholder="Open Close Time" />
                                    </div>
                                </div>
                                        ...
                                        <select name="transaction_type" id="transaction_type"
                                            ...
                                            @foreach (config('app.cash_register_transaction_type') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('transaction_type', @$cash_register->transaction_type) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select name="verified_by" id="verified_by"
                                            ...
                                            @foreach (@$admin as $key => $value)
                                                <option data-kt-flag="{{ $value->id }}"
                                                    value="{{ $value->id }}"
                                                    {{ $value->id == old('verified_by', @$cash_register->verified_by) ? 'selected' : '' }}>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="status" id="status" aria-label="{{ __('Select Status') }}"
                                            ...
                                            @foreach (config('app.statusinactive') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('status', @$cash_register->status) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
