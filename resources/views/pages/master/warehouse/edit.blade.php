<x-default-layout>
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Warehouse'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="warehouse_details" class="collapse show">
            <!--begin::Form-->
            <form id="warehouse_details_form" class="form" method="POST"
                action="{{ route('admin.warehouse.update', $warehouse->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="text" name="slug" value="{{ old('slug', $warehouse->slug) }}"
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="start_date" type="text"
                                        ...
                                        value="{{ old('start_date', $warehouse->start_date) }}" />
                                        ...
                                        <select name="country_id" id="country-dropdown"
                                            ...
                                            @foreach ($countries as $key => $value)
                                                <option data-kt-flag="{{ $value->id }}"
                                                    value="{{ $value->id }}"
                                                    {{ $value->id == old('country_id', $warehouse->country_id) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="text" name="direction"
                                    value="{{ old('direction', $warehouse->direction) }}"
                                        ...
                                        <select name="status" id="status" aria-label="{{ __('Select Status') }}"
                                            ...
                                            @foreach (config('app.statusinactive') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('status', $warehouse->status) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                <label for="is_default"
                                    class="form-label required">{{ __('Is Default Warehouse') }}</label>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="is_default" id="is_default"
                                            ...
                                            @foreach (config('app.yesorno') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('is_default', $warehouse->is_default) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
