<x-default-layout>
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_2' => 'Edit Payment Type',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Payment Type'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="paymenttype" class="collapse show">
            <!--begin::Form-->
            <form id="paymenttype_form" class="form" method="POST"
                action="{{ route('admin.payment-type.update', $paymenttype->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                <div class="card-body border-top p-9">
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <input type="text" name="payment_type"
                                    ...
                                    value="{{ old('payment_type', $paymenttype->payment_type) }}" required />
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <input type="text" name="slug"
                                    class="form-control form-control-solid form-control-lg mb-3 mb-lg-0"
                                    placeholder="Slug" value="{{ old('slug', $paymenttype->slug) }}" required />
                                @if ($errors->has('slug'))
                                    <span class="fv-plugins-message-container invalid-feedback" role="alert">
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <input type="text" name="machine_number"
                                    ...
                                    value="{{ old('machine_number', $paymenttype->machine_number) }}" />
                                ...
                                <select name="payment_category" id="payment_category"
                                    ...
                                    @foreach (config('app.payment_category') as $payment_category)
                                        <option data-kt-flag="{{ $payment_category['value'] }}"
                                            ...
                                            {{ $payment_category['value'] == old('payment_category', $paymenttype->payment_category) ? 'selected' : '' }}>
                                ...
                                <select name="store_id" id="store_id" aria-label="{{ __('Select Store') }}"
                                    ...
                                    @foreach ($stores as $key => $store)
                                        <option value="{{ $store->id }}"
                                            {{ $store->id == old('store_id', $paymenttype->store_id) ? 'selected' : '' }}
                    ...
                    <div class="row mb-6">
                        <!--begin::Label-->
                        <label class="col-lg-4 col-form-label fw-bold fs-6">{{ __('Icon') }}
                            <span>{!! $paymenttype->file != null
                                ? '<a href=' . $paymenttype->image_full_url . " target='_blank'><i class='fa fa-eye'></i></a>"
                                : '' !!}</span></label>
                        <!--end::Label-->
                        ...
                        <div class="col-lg-8 d-flex align-items-center">
                            <div class="form-check form-check-solid form-check-custom form-switch fv-row">
                                ...
                                <input class="form-check-input w-45px h-30px" type="checkbox" id="status"
                                    name="status" value="1"
                                    {{ old('status', $paymenttype->status) ? 'checked' : '' }} />
