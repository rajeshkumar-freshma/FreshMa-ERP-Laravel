<x-default-layout>
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_2' => 'Add Machine Details',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Add Machine Details'])
        <!--begin::Card header-->
        ...
        <div id="item_details" class="collapse show">
            ...
            <form id="category_details_form" class="form" method="POST"
                action="{{ route('admin.machine-details.store') }}" enctype="multipart/form-data">
                @csrf
                <!--begin::Card body-->
                                        ...
                                        <select name="plu_master_code" id="plu_master_code"
                                            ...
                                            @foreach (@$plu_master_datas as $key => $plu_master_data)
                                                <option value="{{ $plu_master_data->plu_master_code }}"
                                                    {{ $plu_master_data->plu_master_code == old('plu_master_code') ? 'selected' : '' }}>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="machine_status" id="machine_status"
                                            ...
                                            @foreach (config('app.machine_status') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('machine_status') ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
