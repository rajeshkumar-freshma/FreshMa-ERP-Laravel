<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            'title' => 'Edit Staff Attendence',
            'menu_1_link' => route('admin.staff_attendance.index'),
            'menu_2' => 'Edit Staff Attendence',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Staff Attendance'])
        <!--begin::Card header-->
        ...
        <div id="employee_details" class="collapse show">
            ...
            <form id="employee_details_form" class="form" method="POST"
                action="{{ route('admin.staff_attendance.update', @$staff_attendance->id) }}"
                ...
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="mb-5">
                                <label for="attendance_date" class="col-lg-4 col-form-label required fw-bold fs-6">{{ __('Select Attendance Date') }}</label>
                                <div class="input-group">
                                    <input id="attendance_date" type="date"
                                        ...
                                        value="{{ old('attendance_date', isset($staff_attendance) ? \Carbon\Carbon::parse($staff_attendance->attendance_date)->format('Y-m-d') : '') }}" />
                                </div>
                                ...
                                <select name="store_id" id="store_id" aria-label="{{ __('Select Store') }}"
                                    ...
                                    @foreach ($store as $key => $details)
                                        <option value="{{ $details->id }}"
                                            {{ $details->id == @$staff_attendance->store_id ? 'selected' : '' }}
                                            class="optionGroup">
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                <label onchange="getEmployees()"
                                    class="col-lg-4 col-form-label required fw-bold fs-6">{{ __('Select Store') }}</label>
                                <input type="text" name="staff_attendance_id" id="staff_attendance_id"
                                    value="{{ @$staff_attendance->id }}" hidden>
                                <select name="store_id" id="store_id" aria-label="{{ __('Select Store') }}"
                                    data-control="select2" data-placeholder="{{ __('Select Store..') }}"
                                    class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                    <option value="">Select Store</option>
                                    ...
                                    @foreach (config('app.attendance_type') as $type)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="attendance_type"
                                                ...
                                                {{ @$staff_attendance_details->first()->is_present == $type['value'] ? 'checked' : '' }}>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <input type="datetime-local" name="in_datetime"
                                            value="{{ old('in_datetime', @$staff_attendance_details[0]->in_time ?? '') }}"
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <input type="datetime-local" name="out_datetime"
                                            value="{{ old('out_datetime', @$staff_attendance_details[0]->out_time ?? '') }}"
                                        ...
                                        <select name="status" id="status" aria-label="{{ __('Select Status') }}"
                                            ...
                                            @foreach (config('app.statusinactive') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ @$staff_attendance->status == $value['value'] ? 'selected' : '' }}>
                                                    {{ $value['name'] }}
                                                </option>
                                            @endforeach
                                        </select>
