<x-default-layout>
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Leave'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="leave" class="collapse show">
            <!--begin::Form-->
            <form id="leave_form" class="form" method="POST" action="{{ route('admin.leave.update', $leave->id) }}"
                                        ...
                                        <select name="employee_id" id="employee_id"
                                            ...
                                            @foreach ($employee as $key => $details)
                                                <option value="{{ $details->id }}"
                                                    {{ $details->id == $leave->employee_id ? 'selected' : '' }}
                                                    class="optionGroup">
                                                    {{ ucFirst($details->first_name) }}
                                                    {{ ucFirst($details->last_name) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="leave_type" id="leave_type"
                                            ...
                                            @foreach ($leave_type as $key => $details)
                                                <option value="{{ $details->id }}"
                                                    {{ $details->id == $leave->leave_type ? 'selected' : '' }}
                                                    class="optionGroup">
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <!-- Replace select with date picker input -->
                                            ...
                                            name="start_date" placeholder="Enter Start Date"  value={{ $leave->start_date != null ? $leave->start_date : '' }}  required />
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <!-- Replace select with date picker input -->
                                            ...
                                            class="form-control form-control-solid fsh_flat_datepicker"
                                            name="end_date" placeholder="Enter End Date"value={{ $leave->end_date != null ? $leave->end_date : '' }} required />
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <!-- Replace select with date picker input -->
                                        <textarea name="reason" class="form-control form-control-sm form-control-solid" id="reason"
                                            placeholder="Enter Reason" required>{{ $leave->reasons }}</textarea>
                                    </div>
                                </div>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        ...
                                        <textarea name="remark" class="form-control form-control-sm form-control-solid" id="remark"
                                            placeholder="Enter Remark">{{ $leave->remark }}</textarea>
                                    </div>
                                </div>
                                        ...
                                        <select name="approved_status" id="approved_status"
                                            ...
                                            @foreach (config('app.approved_status') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $leave->approved_status == $value['value'] ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="row mb-0">
                        ...
                        <div class="col-lg-8 d-flex align-items-center">
                            <div class="form-check form-check-solid form-check-custom form-switch fv-row">
                                ...
                                <input class="form-check-input w-45px h-30px" type="checkbox" id="is_half_day"
                                    name="is_half_day" value="1"
                                    {{ $leave->is_half_day == 1 ? 'checked' : '' }} />
