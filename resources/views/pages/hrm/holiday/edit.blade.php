<x-default-layout>
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Holiday'])
        <!--begin::Card header-->
        ...
        <div id="holiday" class="collapse show">
            ...
            <form id="holiday_form" class="form" method="POST"
                action="{{ route('admin.holiday.update', @$holiday->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <input type="text" name="name"
                                    ...
                                    placeholder="Name" value="{{ old('name', @$holiday->name) }}" required />
                                @if ($errors->has('name'))
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    <div class="overflow-hidden flex-grow-1">
                                        <input type="text" name="date" id="date"
                                            ...
                                            value="{{ old('date', date('Y-m-d', strtotime(@$holiday->date))) }}"
                                            required>
                                    </div>
                                        ...
                                        <select name="holiday_type" id="holiday_type"
                                            ...
                                            @foreach (config('app.holiday_type') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}"
                                                    value="{{ $value['value'] }}"
                                                    {{ $value['value'] == old('holiday_type', @$holiday->holiday_type) ? 'selected' : '' }}>
                                                    {{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="col-lg-8 d-flex align-items-center">
                            <div class="form-check form-check-solid form-check-custom form-switch fv-row">
                                ...
                                <input class="form-check-input w-45px h-30px" type="checkbox" id="status"
                                    name="status" value="1"
                                    {{ old('status', @$holiday->status) ? 'checked' : '' }} />
