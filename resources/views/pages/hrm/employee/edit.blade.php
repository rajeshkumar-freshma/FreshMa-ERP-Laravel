<x-default-layout>
    @section('_toolbar')
        @php
            $data['title'] = 'Employee';
            $data['menu_1'] = 'Employee';
            $data['menu_1_link'] = route('admin.employee.index');
            $data['menu_2'] = 'Edit';
        @endphp
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Employee'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="employee_details" class="collapse show">
            <!--begin::Form-->
            <form id="employee_details_form" class="form" method="POST" action="{{ route('admin.employee.update', $staff->id) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <!--begin::Card body-->
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="text" name="last_name" value="{{ old('last_name', $staff->last_name) }}" class="form-control form-control-sm form-control-solid" id="last_name" placeholder="Enter Name"/>
                        ...
                        <div class="col-md-6">
                            <div class="mb-5">
                                ...
                                <input type="email" name="email" value="{{ old('email', $staff->email) }}" class="form-control form-control-sm form-control-solid" id="email" placeholder="Enter Email"/>
                                @if ($errors->has('email'))
                                    <span class="fv-plugins-message-container invalid-feedback" role="alert">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-5">
                                <label for="status" class="form-label ">{{ __('Profile Photo') }}</label>
                                <br>
                ...
                <hr>
                @include('pages.hrm.employee.employee_documents', ['staff_user_info' => $staff->user_info])

                <hr>
                @if (old('employee_store'))
                    @include('pages.hrm.employee.employee_detail', ['data' => old('employee_store')])
                @else
                    @include('pages.hrm.employee.employee_detail', ['data' => $staff->staff_store_mapping])
                @endif

                <!--end::Card body-->

                <!--begin::Actions-->
                @include('pages.partials.form_footer', ['is_save' => true, 'back_url' => route('admin.employee.index')])
                <!--end::Actions-->
            </form>
            <!--end::Form-->
        </div>
    ...
    <!--end::Basic info-->
    @php
        $country_id = $staff->user_info->country_id;
        $state_id = $staff->user_info->state_id;
        $city_id = $staff->user_info->city_id;
    @endphp
    @section('scripts')
        @include('pages.partials.state_city_script')
        @include('pages.partials.date_picker')
        @include('pages.hrm.employee.script')
    @endsection
</x-default-layout>
