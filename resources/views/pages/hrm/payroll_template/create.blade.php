<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            'title' => 'Add Payroll Template',
            'menu_1_link' => route('admin.pay-roll-template.index'),
            'menu_2' => 'Add Payroll Template',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Add Payroll Template'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="payroll_template" class="collapse show">
            <!--begin::Form-->
            <form id="payroll_template_form" class="form" method="POST"
                action="{{ route('admin.pay-roll-template.store') }}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-lg-8">
                            <div class="row" id="payrollTypesContainer">
                                @forelse (@$payroll_types as $key => $details)
                                    <div class="row mb-2">
                                        <label
                                            class="col-md-3 col-form-label {{ $details->payroll_types == 1 ? 'text-success' : 'text-danger' }}">
                                            {{ ucfirst($details->name) }}
                                        </label>
                                        <input type="hidden" name="payroll_data[{{ $details->id }}][payroll_type_id]"
                                            value="{{ $details->id }}">
                                        <div class="col-md-3">
                                            <input name="payroll_data[{{ $details->id }}][amount]" type="text"
                                                class="form-control" placeholder="Enter Amount" required>
                                                @if ($errors->has("payroll_data.$details->id.amount"))
                                                <span class="fv-plugins-message-container invalid-feedback" role="alert">
                                                    <strong>{{ $errors->first("payroll_data.$details->id.amount") }}</strong>
                                                </span>
                                            @endif
                                        </div>
                                        <div class="col-md-3">
                                    ...
                                    @empty
                                    <p>There is no payroll types on this feild</p>
                                @endforelse
                            </div>
