            ...
            <table class="table align-middle table-row-dashed fs-6 gy-3">
                ...
                <tbody class="fw-bold text-gray-600">
                    {{-- @foreach (@$products as $product)
                        <tr>
                            <td>{{ $product->name }}</td>
                            @foreach (@$warehouseProductsData as $warehouse)
                                <td>
                                    @php
                                        $warehouseStock = $warehouse->warehouse_inventory_details->firstWhere(
                                            'product_id',
                                            $product->id,
                                        );
                                        echo $warehouseStock ? $warehouseStock->weight : '0.00'; // Changed 'weight' to 'total_weight'
                                    @endphp
                                    <p></p>
                                </td>
                            @endforeach
                        ...
                        <tr>
                            ...
                            @foreach ($warehouses as $key => $warehouse)
                                    <td>
                                        ...
                                        @if (in_array($warehouse->id, $product->warehouseInventoryDetails->pluck('warehouse_id')->toArray()))
                                             @php 
                                                $index = array_search($warehouse->id, $product->warehouseInventoryDetails->pluck('warehouse_id')->toArray());
                                                $value = $product->warehouseInventoryDetails->pluck('warehouse_id')->toArray()[$index];
                                            
                                                 @endphp
                                                {{  $product->warehouseInventoryDetails->where('warehouse_id', $value )->first()->weight}}
                                        @else 
                                            0
                                        @endif
                                    </td>
                    ...
                    @foreach ($products as $product)
                        <tr>
                            ...
                            @foreach ($warehouses as $warehouse)
                                <td>
                                    @php
                                        $inventoryDetail = $product->warehouseInventoryDetails
