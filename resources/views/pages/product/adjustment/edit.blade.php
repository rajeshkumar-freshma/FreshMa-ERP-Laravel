<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_2' => 'Edit Adjustment',
        ])
    @endsection
    <!--begin::Basic info-->
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Edit Adjustment'])
        <!--begin::Card header-->

        <!--begin::Content-->
        <div id="item_details" class="collapse show">
            <!--begin::Form-->
            <form id="item_details_form" class="form" method="POST" action="{{ route('admin.adjustment.update', $adjustment->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <input type="text" class="form-control form-control-sm form-control-solid" id="adjustment_track_number" name="adjustment_track_number" value="{{ old('adjustment_track_number', $adjustment->adjustment_track_number) }}" placeholder="SIR-" required />
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="adjustment_date" class= form-label">{{ __('Adjustment Date') }}</label>
                                <div class="input-group">
                                    <input id="adjustment_date" type="text" class="form-control form-control-sm form-control-solid fsh_flat_datepicker" name="adjustment_date" value="{{ old('adjustment_date', $adjustment->adjustment_date) }}" />
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="warehouse_id" id="ir_warehouse" aria-label="{{ __('Select Warehouse') }}" data-control="select2" data-placeholder="{{ __('Select Warehouse..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            @foreach ($warehouses as $key => $warehouse)
                                                <option data-kt-flag="{{ $warehouse->id }}" value="{{ $warehouse->id }}" {{ $warehouse->id == old('warehouse_id', $adjustment->warehouse_id) ? 'selected' : '' }}> {{ $warehouse->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="store_id" id="ir_store" aria-label="{{ __('Select Store') }}" data-control="select2" data-placeholder="{{ __('Select Store..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            ...
                                            @foreach ($stores as $key => $store)
                                                <option data-kt-flag="{{ $store->id }}" value="{{ $store->id }}" {{ $store->id == old('store_id', $adjustment->store_id) ? 'selected' : '' }}> {{ $store->store_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="status" id="po_status" aria-label="{{ __('Select Status') }}" data-control="select2" data-placeholder="{{ __('Select Status..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            @foreach (config('app.adjustment_status') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}" value="{{ $value['value'] }}" {{ $value['value'] == old('status', $adjustment->status) ? 'selected' : '' }}>{{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="ir_file" class="form-label">{{ __('Attachments') }} {!! commoncomponent()->attachment_view($adjustment->image_full_url) !!}</label>
                    ...
                    @include('pages.partials.product_search.index', ['loop_data' => old('products', @$adjustment->adjustment_details), 'main_data' => $adjustment])
                    <!--End::Product Search-->
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-md-12">
                            <div class="mb-5">
                                ...
                                <textarea class="form-control form-control-sm form-control-solid" name="remarks" id="remarks" cols="30" rows="3">{{ old('remarks', $adjustment->remarks) }}</textarea>
                            </div>
                        </div>
                        <!--begin::Label-->
                    </div>
