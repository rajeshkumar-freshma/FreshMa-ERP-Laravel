<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            'title' => 'Add Bulk Product Transfer',
            'menu_1_link' => route('admin.bulk-product-transfer.index'),
            'menu_2' => 'Add Bulk Product Transfer',
        ])
    @endsection
    <!--begin::Basic info-->
    <form id="product_transfer_form" class="form" method="POST" action="{{ route('admin.bulk-product-transfer.store') }}"
      ...
      @method('post')
                ...
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="mb-5">
                                @php
                                    $invoice_number = commoncomponent()->invoice_no('product_bulk_transfer');
                                @endphp
                                ...
                                <input type="text" class="form-control form-control-sm form-control-solid"
                                    ...
                                    value="{{ old('transfer_order_number', $invoice_number) }}" placeholder="PO Number"
                                    required />
                                ...
                                @endif
                                <div id="final_submission_type"></div>
                            </div>
                        </div>

                        <div class="col-md-4 from_warehouse_div">
                            <div class="mb-5">
                                <label for="from_warehouse_id" class="required form-label">{{ __('From Warehouse') }}</label>
                                        ...
                                        <select name="from_warehouse_id" id="from_warehouse_id"
                                            ...
                                            @foreach ($warehouses as $key => $warehouse)
                                                <option data-kt-flag="{{ $warehouse->id }}" value="{{ $warehouse->id }}"
                                                    {{ $warehouse->id == old('from_warehouse_id') ? 'selected' : '' }}>
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="transfer_created_date" type="text"
                                        ...
                                        value="{{ old('transfer_created_date', date('Y-m-d')) }}"
                            ...
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="transfer_received_date" type="text"
                                        class="form-control form-control-sm form-control-solid fsh_flat_datepicker"
                                        name="transfer_received_date" value="{{ old('transfer_received_date') }}"
        ...
        {{-- @include('pages.partials.form_footer', [
            'is_save' => true,
            'back_url' => route('admin.bulk-product-transfer.index'),
            'is_bulk_product_transfer' => true,
        ]) --}}
        <div class="card mt-2">
            <div class="card-footer d-flex justify-content-end py-6 px-9">
                ...
                <button type="submit" class="btn btn-sm btn-primary me-2 submission_type" id="item_details_submit"
                data-id="{{ config('app.submission_type')[0]['value'] }}"
                 value={{ config('app.submission_type')[0]['value'] }}>
                    @include('partials.general._button-indicator', [
                        'label' => __(config('app.submission_type')[0]['name']),
                    ])
                </button>

                <button type="submit" class="btn btn-sm btn-success me-2 submission_type" id="item_details_submit"
                data-id="{{ config('app.submission_type')[1]['value'] }}"
                 value={{ config('app.submission_type')[1]['value'] }}>
                    @include('partials.general._button-indicator', [
                        'label' => __(config('app.submission_type')[1]['name']),
                    ])
                </button>
                <button type="submit" class="btn btn-sm btn-info submission_type" id="distribution_submit"
                data-id="distribution_submit"
                         >
    ...
    @section('scripts')
        ...
        <script>
            $(function() {
                ...
                $(document).on('click','.submission_type',  function() {
                    let final_submission_type = $(this).data('id');
                    $submission = `<input type="hidden" name="submission_type" value="`+ final_submission_type +`">`
                    $('#final_submission_type').html($submission);
                })
