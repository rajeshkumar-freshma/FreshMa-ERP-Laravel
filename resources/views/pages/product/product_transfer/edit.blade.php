<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_2' => 'Edit Product Transfer',
        ])
    @endsection
    <!--begin::Basic info-->
    <!--begin::Form-->
    <form id="product_transfer_form" class="form" method="POST" action="{{ route('admin.product-transfer.update', $product_transfer->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <!--begin::Card header-->
            @include('pages.partials.form_header', ['header_name' => 'Edit Product Transfer'])
            <!--begin::Card header-->
            <div id="product_transfer">
                ...
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <input type="text" class="form-control form-control-sm form-control-solid" id="transfer_order_number" name="transfer_order_number" value="{{ old('transfer_order_number', $product_transfer->transfer_order_number) }}" placeholder="PO Number" required />
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="transfer_from" id="transfer_from" aria-label="{{ __('Select Transfer From') }}" data-control="select2" data-placeholder="{{ __('Select Transfer From..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            <option value="1" {{ old('transfer_from', $product_transfer->transfer_from) == 1 ? 'selected' : '' }}>{{ __('Warehouse') }}</option>
                                            <option value="2" {{ old('transfer_from', $product_transfer->transfer_from) == 2 ? 'selected' : '' }}>{{ __('Store') }}</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="transfer_to" id="transfer_to" aria-label="{{ __('Select Transfer To') }}" data-control="select2" data-placeholder="{{ __('Select Transfer To..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            <option value="1" {{ old('transfer_to', $product_transfer->transfer_to) == 1 ? 'selected' : '' }}>{{ __('Warehouse') }}</option>
                                            <option value="2" {{ old('transfer_to', $product_transfer->transfer_to) == 2 ? 'selected' : '' }}>{{ __('Store') }}</option>
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <select name="from_warehouse_id" id="from_warehouse_id" aria-label="{{ __('Select Warehouse') }}" data-control="select2" data-placeholder="{{ __('Select Warehouse..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            ...
                                            @foreach ($warehouses as $key => $warehouse)
                                                <option data-kt-flag="{{ $warehouse->id }}"
                                                    value="{{ $warehouse->id }}"
                                                    {{ $warehouse->id == old('from_warehouse_id', $product_transfer->from_warehouse_id) ? 'selected' : '' }}
                                                    {{ $warehouse->id != $product_transfer->from_warehouse_id ? 'disabled' : '' }}>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="from_store_id" id="from_store_id" aria-label="{{ __('Select Store') }}" data-control="select2" data-placeholder="{{ __('Select Store..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            ...
                                            @foreach ($stores as $key => $store)
                                                <option data-kt-flag="{{ $store->id }}" value="{{ $store->id }}" {{ $store->id == old('from_store_id', $product_transfer->from_store_id) ? 'selected' : '' }}> {{ $store->store_name . ' - ' . $store->store_code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="to_warehouse_id" id="to_warehouse_id" aria-label="{{ __('Select Warehouse') }}" data-control="select2" data-placeholder="{{ __('Select Warehouse..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            <option value="">{{ __('Select Warehouse..') }}</option>
                                            @foreach ($warehouses as $key => $warehouse)
                                                <option data-kt-flag="{{ $warehouse->id }}" value="{{ $warehouse->id }}" {{ $warehouse->id == old('to_warehouse_id', $product_transfer->to_warehouse_id) ? 'selected' : '' }}> {{ $warehouse->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="input-group input-group-sm flex-nowrap">
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="to_store_id" id="to_store_id" aria-label="{{ __('Select Store') }}" data-control="select2" data-placeholder="{{ __('Select Store..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            <option value="">{{ __('Select Store..') }}</option>
                                            @foreach ($stores as $key => $store)
                                                <option data-kt-flag="{{ $store->id }}" value="{{ $store->id }}" {{ $store->id == old('to_store_id', $product_transfer->to_store_id) ? 'selected' : '' }}> {{ $store->store_name . ' - ' . $store->store_code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="transfer_created_date" type="text" class="form-control form-control-sm form-control-solid fsh_flat_datepicker" name="transfer_created_date" value="{{ old('transfer_created_date', $product_transfer->transfer_created_date) }}" placeholder="Return Date" required />
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="transfer_received_date" type="text" class="form-control form-control-sm form-control-solid fsh_flat_datepicker" name="transfer_received_date" value="{{ old('transfer_received_date', $product_transfer->transfer_received_date) }}" placeholder="Return Date" required />
                                    <span class="input-group-text border-0">
                                        <i class="fas fa-calendar"></i>
                                    </span>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="status" id="po_status" aria-label="{{ __('Select Status') }}" data-control="select2" data-placeholder="{{ __('Select Status..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            @foreach (config('app.purchase_status') as $key => $value)
                                                <option data-kt-flag="{{ $value['value'] }}" value="{{ $value['value'] }}" {{ $value['value'] == old('status', $product_transfer->status) ? 'selected' : '' }}>{{ $value['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="ir_file" class="form-label">{{ __('Attachments') }} {!! commoncomponent()->attachment_view($product_transfer->image_full_url) !!}</label>
                    ...
                    @include('pages.partials.product_search.index', ['loop_data' => old('products', $product_transfer->product_details), 'main_data' => $product_transfer])
                    <!--End::Product Search-->
        ...
        @if (old('transport_tracking.transport_type_id'))
            ...
            @endphp
        @else
            @include('pages.partials.transport_detail', ['transport_detail' => $product_transfer->transport_details, 'is_old_data' => 'edit_value'])
            @php
                $tracking_count = count($product_transfer->transport_details);
            @endphp
        @endif
        ...
        @if (old('expense.expense_type_id'))
            ...
            @endphp
        @else
            @include('pages.partials.expense_detail', ['data' => $product_transfer->expense_details, 'is_old_data' => 'edit_value'])
            @php
                $expense_count = count($product_transfer->expense_details);
            @endphp
        @endif
        @if (old('payment_details.payment_type_id'))
            ...
            @endif
        <!--begin::Actions-->
        @include('pages.partials.form_footer', ['is_save' => false, 'back_url' => route('admin.product-transfer.index')])
        <!--end::Actions-->
    </form>
    <!--end::Form-->
    <!--end::Basic info-->
