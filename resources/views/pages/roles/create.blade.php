<x-default-layout>
    ...
    @section('_toolbar')
        @include('pages.partials.common-toolbar', [
            ...
            'menu_1_link' => route('admin.assign-role-to-users.index'),
    ...
    <div class="card">
        ...
        <div id="roles" class="collapse show">
            <form id="roles_form" class="form" method="POST"
                action="{{ route('admin.role-management.roles.store') }}" enctype="multipart/form-data">
                @csrf
                <!--begin::Card body-->
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="row">
                                <input type="text" name="role_name" id="role_name"
                                    ...
                                    placeholder="Name" value="{{ old('role_name') }}" required />
                                ...
                                @foreach ($permissionGroup->permissions as $permissions)
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input mt-2" name="permissions_ids[]"
                                            value="{{ old('permissions_ids[]', $permissions->id) }}"
                                            id="permission_{{ $permissions->id }}">
                                        <label class="form-check-label mt-3" for="permission_{{ $permissions->id }}">
                                            {{ $permissions->name }}
                                        </label>
                                    </div>
                                @endforeach
