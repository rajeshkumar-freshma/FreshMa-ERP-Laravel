                ...
                <div class="card-body">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-3" id="kt_table_widget_5_table">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            ...
                            <tr>
                                <!--begin::Product ID-->
                                <td class="text-end">{{$purchase_detail->name}}</td>
                                <!--end::Product ID-->
                                @if ($purchase_detail->unit_id == 2)
                                <td class="text-end">Gram</td>
                                @else<td class="text-end">Kilo Gram</td>
                                @endif

                                <td class="text-end">{{$purchase_detail->request_quantity}}</td>
                                <!--begin::Date added-->
                                <td class="text-end">{{$purchase_detail->given_quantity}}</td>
                                <!--end::Date added-->
                                <!--begin::Status-->
                                <td class="text-end">{{$purchase_detail->amount}}</td>
                                <td class="text-end">{{$purchase_detail->tax_value}}</td>
                                @if ($purchase_detail->discount_type == 1)
                                <td class="text-end" data-order="58">Fixed</td>
                                @else
                                <td class="text-end" data-order="58">Percentage</td>
                                @endif

                                <td class="text-end" data-order="58">{{$purchase_detail->discount_amount}}</td>
                                <td class="text-end" data-order="58">{{$purchase_detail->sub_total}}</td>
                                <!--end::Qty-->
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="d-flex justify-content-between mx-0 row mt-1">
                        <div class="col-sm-12 col-md-6">
                            Showing {{ $purchase_details->firstItem() }} to {{ $purchase_details->lastItem() }} of
                            {{ $purchase_details->total() }} entries
                        </div>
                        <div class="col-sm-12 col-md-6 float-right">
                            {{ $purchase_details->withQueryString()->links('vendor.pagination.custom') }}
                        </div>
                    </div>
                    @endif
                    <!--end::Table-->
