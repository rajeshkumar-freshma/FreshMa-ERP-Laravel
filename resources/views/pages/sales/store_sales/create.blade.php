<x-default-layout>
    ...
    <div class="card">
        <!--begin::Card header-->
        @include('pages.partials.form_header', ['header_name' => 'Add Branch Sale'])
        <!--begin::Card header-->
    </div>
    ...
    <div id="item_details" class="collapse show">
        <!--begin::Form-->
        <form id="item_details_form" class="form" method="POST" action="{{ route('admin.store-sales.store') }}" enctype="multipart/form-data">
            @csrf
            <!--begin::Card body-->
            <div class="card">
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="mb-5">
                                @php
                                    $invoice_number = commoncomponent()->invoice_no('store_sale');
                                @endphp
                                <label for="store_sales_number" class="required form-label">{{ __('Store Sale Number') }}</label>
                                <input type="text" class="form-control form-control-sm form-control-solid" id="store_sales_number" name="store_sales_number" value="{{ old('store_sales_number', $invoice_number) }}" placeholder="Store Sales number-" required />
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                <label for="store_indent_request" class="form-label">{{ __('Store Indent Request') }}</label>

                                <div class="input-group input-group-sm flex-nowrap">
                                    <span class="input-group-text border-0"><i class="fas fa-home"></i></span>
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="store_indent_request_id" id="store_indent_request" aria-label="{{ __('Select Store Indent Request') }}" data-control="select2" data-placeholder="{{ __('Select Store Indent Request..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            <option value="">{{ __('Select Store Indent Request..') }}</option>
                                            @foreach ($store_indent_requests as $key => $store_indent_request)
                                                <option data-kt-flag="{{ $store_indent_request->id }}" value="{{ $store_indent_request->id }}" {{ $store_indent_request->id == old('store_indent_request_id') ? 'selected' : '' }}> {{ $store_indent_request->request_code . ' - ' . $store_indent_request->store->store_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-5">
                                ...
                                <div class="input-group">
                                    <input id="delivered_date" type="text" class="form-control form-control-sm form-control-solid fsh_flat_datepicker" name="delivered_date" value="{{ old('delivered_date') }}" required />
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="store_id" id="store_id" aria-label="{{ __('Select Store') }}" data-control="select2" data-placeholder="{{ __('Select Store..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            @foreach ($stores as $key => $store)
                                                <option data-kt-flag="{{ $store->id }}" value="{{ $store->id }}" {{ $store->id == old('store_id') ? 'selected' : '' }}> {{ $store->store_name . ' - ' . $store->store_code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
            ...
            <!--begin::Actions-->
            @include('pages.partials.form_footer', ['is_save' => true, 'back_url' => route('admin.store-sales.index')])
            <!--end::Actions-->
        </form>
        <!--end::Form-->
    </div>
