<x-default-layout>
    ...
    <div id="item_details" class="collapse show">
        <!--begin::Form-->
        <form id="item_details_form" class="form" method="POST" action="{{ route('admin.customer-sales.store') }}" enctype="multipart/form-data">
            @csrf
            <!--begin::Card body-->
            <div class="card">
                <div class="card-body border-top px-9 py-4">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            <div class="mb-5">
                                @php
                                    $invoice_number = commoncomponent()->invoice_no('sale_order');
                                @endphp
                                <label for="vendor_sales_number" class="required form-label">{{ __('Vendor Sale Number') }}</label>
                                <input type="text" class="form-control form-control-sm form-control-solid" id="vendor_sales_number" name="vendor_sales_number" value="{{ old('vendor_sales_number', $invoice_number) }}" placeholder="VSR-" required />
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="vendor_indent_request_id" id="indent_request" aria-label="{{ __('Select Vendor Indent Request') }}" data-control="select2" data-placeholder="{{ __('Select Vendor Indent Request..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true">
                                            ...
                                            @foreach ($vendor_indent_requests as $key => $vendor_indent_request)
                                                <option data-kt-flag="{{ $vendor_indent_request->id }}" value="{{ $vendor_indent_request->id }}" {{ $vendor_indent_request->id == old('vendor_indent_request_id') ? 'selected' : '' }}> {{ $vendor_indent_request->request_code . ' - ' . $vendor_indent_request->vendor->first_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-hidden flex-grow-1">
                                        <select name="vendor_id" id="ir_vendor" aria-label="{{ __('Select Vendor') }}" data-control="select2" data-placeholder="{{ __('Select Vendor..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                                            ...
                                            @foreach ($vendors as $key => $vendor)
                                                <option data-kt-flag="{{ $vendor->id }}" data-vendor_percentage="{{ $vendor->vendor_detail!=null ? $vendor->vendor_detail->vendor_commission : 0 }}" data-editable="{{ $vendor->vendor_detail!=null ? $vendor->vendor_detail->it_can_edit_on_billing : 0 }}" value="{{ $vendor->id }}" {{ $vendor->id == old('vendor_id') ? 'selected' : '' }}> {{ $vendor->first_name . ' - ' . $vendor->last_name . ' - ' . $vendor->user_code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
            ...
            <!--begin::Actions-->
            @include('pages.partials.form_footer', ['is_save' => true, 'back_url' => route('admin.customer-sales.index')])
            <!--end::Actions-->
        </form>
        <!--end::Form-->
    </div>
