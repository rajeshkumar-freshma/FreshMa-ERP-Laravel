@foreach ($indent_request_details as $key => $indent_request_detail)
    @php
        $count = $key;
    @endphp
    <tr class="product_row current_row{{ $key }}">
        ...
        <td>
            ...
            <input type="hidden" class="form-control form-control-sm product" data-loopkey="{{ $count }}" value="{{ @$indent_request_detail->product->id }}" name="products[product_id][]" required>
            {{ __($indent_request_detail->product->sku_code != null ? $indent_request_detail->product->name . ' - ' . $indent_request_detail->product->sku_code : $indent_request_detail->product->name) }}
        </td>
            ...
            <td>
                <select name="products[unit_id][]" id="unit_id{{ $count }}" data-loopkey="{{ $count }}" aria-label="{{ __('Search Unit') }}" data-control="select2" data-placeholder="{{ __('Search Unit..') }}" class="form-select form-select-sm form-select-solid" data-allow-clear="true" required>
                    ...
                    @foreach ($units as $key => $unit)
                        <option data-kt-flag="{{ $unit->id }}" value="{{ $unit->id }}" {{ $unit->id == old('unit_id', @$indent_request_detail->unit_id != null ? $indent_request_detail->unit_id : $indent_request_detail->product->unit_id) ? 'selected' : '' }}>{{ $unit->unit_short_code }}</option>
                    @endforeach
                </select>
            </td>
        @endif
        ...
        <td>
            <input type="text" class="form-control form-control-sm quantity" id="quantity{{ $count }}" data-loopkey="{{ $count }}" name="products[quantity][]" value="{{ $indent_request_detail->request_quantity != null ? $indent_request_detail->request_quantity : $indent_request_detail->quantity }}" style="width: 100px;" required>
        </td>

        @if ($given_quantity_display == 'true')
            <td>
                <input type="text" class="form-control form-control-sm given_quantity" id="given_quantity{{ $count }}" data-loopkey="{{ $count }}" name="products[given_quantity][]" value="{{ isset($indent_request_detail->request_quantity) ? $indent_request_detail->request_quantity : $indent_request_detail->quantity }}" style="width: 100px;" required>
            </td>
        @endif

        @if ($amountdisplay == 'true')
            <td>
                <input type="text" class="form-control form-control-sm amount" id="amount{{ $count }}" data-loopkey="{{ $count }}" name="products[amount][]" value="{{ $indent_request_detail->amount }}" style="width: 100px;" required>

                <span class="d-flex">Per Unit : <span id="per_unit_amount{{ $count }}">{{ $indent_request_detail->per_unit_price }}</span></span>
                <input type="hidden" class="form-control form-control-sm per_unit_amount_val" id="per_unit_amount_val{{ $count }}" data-loopkey="{{ $count }}" name="products[per_unit_price][]" value="{{ $indent_request_detail->per_unit_price }}" style="width: 100px;">
            </td>
        @endif
        ...
        @if ($taxanddiscountdisplay == 'true')
            ...
            <td>
                ...
                <input type="number" class="form-control form-control-sm discount_percentage" id="discount_percentage{{ $count }}" data-loopkey="{{ $count }}" name="products[discount_percentage][]" placeholder="Discount Percentage" style="width: 100px; display:none" min="0" max="100">
        ...
        @endif
        <td><input type="number" id="box_no" class="box_no"></td>
        <td>
            <button type="button" class="btn btn-sm btn-danger remove_item" data-id="{{ $indent_request_detail->product->id }}"><i class="fa fa-remove"></i></button>
        </td>
    </tr>
@endforeach
