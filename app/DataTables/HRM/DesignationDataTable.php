...
class DesignationDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function (Designation $model) {
                $status = $model->status;
                return view('pages.partials.statuslabel', compact('status'));
            })
            ->editColumn('created_at', function (Designation $model) {
                return CommonComponent::getCreatedAtFormat($model->created_at);
            })
            ->addColumn('action', function (Designation $model) {
                return view('pages.hrm.designation._action-menu', compact('model'));
            });
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (Auth::check() && Auth::user()->can('Desigination Create')) {
