...
class LeaveTypeDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function (LeaveType $model) {
                $status = $model->status;
                return view('pages.partials.statuslabel', compact('status'));
            })
            ->editColumn('created_at', function (LeaveType $model) {
                return CommonComponent::getCreatedAtFormat($model->created_at);
            })
            ->addColumn('action', function (LeaveType $model) {
                return view('pages.hrm.leave_type._action-menu', compact('model'));
            });
    }
    ...
    public function html(): HtmlBuilder
    {
        ...
        if (Auth::check() && Auth::user()->can('Leave Type Create')) {
        ...
        return $this->builder()
            ...
            ->minifiedAjax('', 'data.date_from = $("#leavetype-table-date-from").val(); data.date_to = $("#leavetype-table-date-to").val(); data.status = $("#leavetype-table-status-filter").val();')
            ->stateSave(false)
            ->responsive()
            ->autoWidth(true)
        // ->selectStyleSingle()
