...
class AccountDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('account_type', function (Account $model) {
                $account_type = $model->account_type;
                return view('pages.partials.statuslabel', compact('account_type'));
            })
            ->editColumn('status', function (Account $model) {
                $status = $model->status;
                return view('pages.partials.statuslabel', compact('status'));
            })
            ->addColumn('action', function (Account $model) {
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
            ...
            ->minifiedAjax('', 'data.date_from = $("#accounts-table-date-from").val(); data.date_to = $("#accounts-table-date-to").val(); data.status = $("#accounts-table-status-filter").val();')
            ->stateSave(false)
            ->responsive()
            ->autoWidth(true)
        // ->selectStyleSingle()
